...
import * as line from "@line/bot-sdk";  // ← これ！
...
function ensureFirebase() {
  ...
  try {
    ...
    console.error("[boot] Firebase init error (deferred):", e);
    // ここで throw しない：/health は返せるようにする
  }
}
function getDb() {
  ensureFirebase();
...
async function handleEvent(event) {
  ...
  if (!couponDoc) {
    ...
    const expiresAt = admin.firestore.Timestamp.fromDate(
      new Date(issuedAt.toDate().getTime() + Number(process.env.VALID_HOURS || 48) * 3600 * 1000)
    );
    const code = genCode();
